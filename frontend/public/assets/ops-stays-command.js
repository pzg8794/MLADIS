(function () {
  ...
  function readRuntimeModules() {
    ...
    if (!script || !script.textContent) return fallbackModules;

    try {
      const items = JSON.parse(script.textContent);
      if (!Array.isArray(items)) return fallbackModules;

      const modules = items.map((item) => [item.label, item.description, item.href, item.tone, item.bg]);
      return modules.every(([label, detail, href, tone, bg]) => [label, detail, href, tone, bg].every((value) => typeof value === 'string'))
        ? modules
        : fallbackModules;
    } catch (error) {
