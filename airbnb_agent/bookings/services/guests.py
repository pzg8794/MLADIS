...
class Guest:
    ...
    def __init__(self, profile: CustomerProfile, request=None):
    ...
    def country(self) -> tuple[str, str]:
        return self.COUNTRY_POOL[(self.profile.pk or 0) % len(self.COUNTRY_POOL)]
    ...
    def past_stay_count(self) -> int:
        ...
        if direct is not None or airbnb is not None:
            return int(direct or 0) + int(airbnb or 0)
        return self.profile.booking_inquiries.count() + self.profile.airbnb_guest_records.count()

    def total_spend_cents(self) -> int:
        annotated = getattr(self.profile, "total_spend_cents", None)
        if annotated:
            return int(annotated)
        invoice_total = self.profile.invoices.aggregate(total=Coalesce(Sum("total_cents"), 0))["total"] or 0
        booking_total = self.profile.booking_inquiries.aggregate(total=Coalesce(Sum("total_cents"), 0))["total"] or 0
        return int(invoice_total or booking_total or 0)

    def last_contact_at(self):
        timestamps = [self.profile.updated_at]
        latest_booking = self.profile.booking_inquiries.order_by("-updated_at").first()
        latest_airbnb = self.profile.airbnb_guest_records.order_by("-updated_at").first()
        latest_feedback = self.profile.feedback_entries.order_by("-updated_at").first()
        latest_invoice = self.profile.invoices.order_by("-updated_at").first()
    ...
    def segment_value(self) -> str:
        if self.profile.segment == ClientSegment.BLACKLISTED:
            return "blacklisted"
        if self.profile.segment == ClientSegment.VIP:
            return "vip"
    ...
    def status_value(self) -> str:
        return "blocked" if self.profile.is_blacklisted else "active"
    ...
    def row_projection(self) -> dict[str, Any]:
        ...
        return {
            ...
            "status": _status_payload(self.status_value(), "Blocked" if self.profile.is_blacklisted else "Active"),
    ...
    def profile_projection(self) -> dict[str, Any]:
        return {
            ...
            "guest_since": _date_label(self.profile.created_at),
            "notes": self.profile.notes or "No notes captured yet.",
    ...
    def message_projection(self) -> dict[str, Any]:
        feedback = list(self.profile.feedback_entries.order_by("-created_at")[:2])
        ...
        latest_booking = self.profile.booking_inquiries.order_by("-created_at").first()
        ...
        if not messages:
            messages.append(
                {
                    "id": f"welcome-{self.profile.pk}",
                    "sender_label": "MLADIS",
                    "body": "Welcome to MLADIS. Share arrival details, guest count, and check-in preferences when ready.",
                    "status": "draft",
    ...
    def documents_projection(self) -> list[dict[str, Any]]:
        ...
        for record in self.profile.airbnb_guest_records.order_by("-created_at")[:3]:
            documents.append(
                {
    ...
    def stays_projection(self) -> list[dict[str, Any]]:
        stays = []
        for booking in self.profile.booking_inquiries.select_related("item").order_by("-check_in", "-updated_at")[:8]:
        ...
        for record in self.profile.airbnb_guest_records.select_related("item").order_by("-check_in", "-updated_at")[:4]:
    ...
    def payments_projection(self) -> dict[str, Any]:
        invoices = list(self.profile.invoices.order_by("-issue_date", "-created_at")[:8])
        items = [
            {
        ...
        holds = ReservationPaymentHold.objects.filter(
            Q(email__iexact=self.profile.email) | Q(inquiry__customer_profile=self.profile)
        ).select_related("inquiry").order_by("-created_at")[:4]
        for hold in holds:
            items.append(
                {
    ...
    def deposits_projection(self) -> dict[str, Any]:
        deposits = list(
            DamageDeposit.objects.filter(Q(email__iexact=self.profile.email) | Q(inquiry__customer_profile=self.profile))
    ...
    def activity_projection(self) -> list[dict[str, Any]]:
        ...
        for booking in self.profile.booking_inquiries.order_by("-created_at")[:4]:
            events.append(
                {
        ...
        for feedback in self.profile.feedback_entries.order_by("-created_at")[:3]:
            events.append(
                {
    ...
    def _birthday(self) -> str:
        day = ((self.profile.pk or 1) % 27) + 1
        month = ((self.profile.pk or 1) % 11) + 1
        year = 1980 + ((self.profile.pk or 1) % 18)
        return date(year, month, day).strftime("%b %-d, %Y")

    def _travel_style(self) -> str:
        styles = ("Leisure", "Family stay", "Business", "Remote work")
        return styles[(self.profile.pk or 0) % len(styles)]
...
class GuestAnalyticsService:
    def compute_segment(self, guest: Guest) -> dict[str, str]:
        if guest.profile.is_blacklisted:
    ...
    def workspace_metrics(self, guests: list[Guest]) -> list[dict[str, Any]]:
        ...
        blocked = sum(1 for guest in guests if guest.profile.is_blacklisted)
    ...
    def segment_tabs(self, guests: list[Guest]) -> list[dict[str, Any]]:
        return [
            ...
            {"value": "blocked", "label": "Blocked", "count": sum(1 for guest in guests if guest.profile.is_blacklisted)},
        ]
