...
- Use `./run_mladis_live.command` from the repo root for the working local Django app.
- Treat `http://127.0.0.1:8000` as the sign-in source of truth.
...
- The local launcher caches dependency installs by `requirements.txt`, skips local `collectstatic` unless `MLADIS_COLLECTSTATIC=1`, and defaults to a 60 second startup timeout.
...
- Use a Cloudflare HTTPS tunnel for Facebook local callbacks and set `SOCIAL_AUTH_FACEBOOK_ORIGIN` to the active tunnel origin.
