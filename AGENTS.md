...
- Use a Cloudflare HTTPS tunnel for Facebook local callbacks. The launcher exports `SOCIAL_AUTH_FACEBOOK_ORIGIN` to the active quick-tunnel URL for the Django process it starts unless an explicit shell override is provided.
- Do not validate Facebook from plain `http://127.0.0.1:8000`; Meta requires the active HTTPS tunnel callback.
